...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.listContactsManyStmt, err = db.PrepareContext(ctx, listContactsMany); err != nil {
		return nil, fmt.Errorf("error preparing query ListContactsMany: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.listContactsManyStmt != nil {
		if cerr := q.listContactsManyStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listContactsManyStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		listContactsManyStmt: q.listContactsManyStmt,
